...
```{r}
whale_shark_weather %>% na.omit() %>% 
  group_by(Month = month(eventDate)) %>%
  summarise(Sightings = sum(sighting),
            tmin_avg = sum(tmin)/n(),
            tmax_avg = sum(tmax)/n(),) %>% 
  ggplot() +
  geom_line(aes(x = Month, y = tmin_avg, color = "blue")) +
  geom_line(aes(x = Month, y = tmax_avg, color = "red")) +
  geom_line(aes(x = Month, y = Sightings, color = "green"))
