...
library(gridExtra)
library(rworldmap)
library(tsibble) # update
library(lubridate) # update
library(rmapshaper) # update
library(zoo) # update
```
...
### Finding nearest tourist destination

Similarly to weather stations, we make use of the same geospatial object of Australia to plot all available tourist destinations in Australia at first from which we will then identify the tourist spot required.

```{r}
#| label: fig-alltourism
#| fig-cap: "All Tourist Destinations"
#| fig-width: 8
#| fig-heigth: 8
#| echo: false
#| warning: false
...
polygon_geometry <- st_geometry(t_region)[1]
...
states %>%
  ...
  ggtitle("Tourism Spots in Entire Australia") 
...
From the plot above showing all tourist spots, we plot the tourist spot which contains the weather station and the sightings with the most occurrences of whale sharks which in this case is **Exmouth** which we were able to figure out by.... We also filtered out the Purpose of tourism visit of Holiday since.....
...
## Temporal analysis on tourism


### Whale Sharks Sightings For a Weather Station Against Tourism Visits

We first start off by visualizing how the overall tourist visits were per quarter for each years by the figure below.
...
highest_sightings %>%
  mutate(quarter = substr(year_quarter, nchar(year_quarter), nchar(year_quarter) + 2),
         year = substr(year_quarter, nchar(year_quarter)-1, nchar(year_quarter) )) %>%
  ggplot(aes(x=year_quarter,y=avg_trips)) +
  geom_line(aes(group=year),alpha=0.3) +
              geom_smooth(se=FALSE)
```
---------Ash's code----------------------
Analysis using scagnostics
...
# Compute Scagnostics
scagnostics_results <- scagnostics(highest_sightings$year_quarter, highest_sightings$avg_trips)

# Print results
print(scagnostics_results)

# Visualize the relationship with a scatterplot
ggplot(highest_sightings, aes(x = year_quarter, y = avg_trips, color = avg_trips)) +
  geom_point() +
  labs(title = "Scatterplot of year_quarter vs avg_trips",
       x = "Year Quarter",
       y = "Average Trips") +
  theme_classic()
...
```{r}
# Assuming the required libraries have been loaded and the dataset is present
library(ggplot2)
library(alphahull)
library(igraph)

# Extract relevant columns from highest_sightings
data <- as.matrix(highest_sightings[, c("year_quarter", "avg_trips")])

# Convex hull
convex_indices <- chull(data)
convex_data <- data[convex_indices, ]

# Plotting
plot(data, pch=19, col="blue", xlab="Year Quarter", ylab="Average Trips")
polygon(convex_data, col=NA, border="red")

# Alpha hull
alpha_obj <- ashape(data, alpha=1)
plot(alpha_obj, add=TRUE, lwd=2)

# MST
distances <- dist(data)
graph <- graph_from_adjacency_matrix(as.matrix(distances), mode="undirected", weighted=TRUE, diag=FALSE)
mst_graph <- mst(graph)
plot(mst_graph, layout=layout.auto(mst_graph))
...
scags_res <- scagnostics(highest_sightings$year_quarter, highest_sightings$avg_trips)
print(scags_res)
...
scagnostics_df <- data.frame(Metric = names(scags_res$s), Value = as.numeric(scags_res$s))

# Sort by "Outlying"
outlying_sorted <- scagnostics_df %>% arrange(desc(Value[Metric == "Outlying"]))

# Sort by "Skewed"
skewed_sorted <- scagnostics_df %>% arrange(desc(Value[Metric == "Skewed"]))

# Sort by "Clumpy"
clumpy_sorted <- scagnostics_df %>% arrange(desc(Value[Metric == "Clumpy"]))
...
ts_data_trips <- ts(highest_sightings$avg_trips, frequency=4)

# Decompose the time series for average trips
decomposition_trips <- stl(ts_data_trips, s.window="periodic")

# Plot the decomposition
autoplot(decomposition_trips) + labs(title = "Seasonal Decomposition of Average Trips")
...
result <- trend::mk.test(highest_sightings$avg_trips)
...
exmouth_2010 <- exmouth[exmouth$year_quarter >= as.yearqtr("2010 Q1"), ]

# Convert the columns for year and quarter to characters for ease of plotting
exmouth_2010$year_quarter <- as.character(exmouth_2010$year_quarter)


# Combine a bar chart and a line chart to observe the trends between the two
ggplot(exmouth_2010) +
  geom_col(aes(x = year_quarter, y = n, fill = "Occurrences"), color = "lightblue") +
  geom_line(aes(x = year_quarter, y = avg_trips, color = "Trips", group=1), size = 0.5) +
  labs(x = "Time (Quarterly)", 
       title = "The Relationship between Tourism and Whale Shark in Exmouth") +
  theme_bw() +
  scale_fill_manual(values = "lightblue", name = "Occurrences", labels = c("The Number of Whale Shark")) +
  scale_color_manual(values = "darkblue", name = "Trips (in thousand)", labels = c("The Number of Tourists")) +
   scale_x_discrete(breaks = exmouth$year_quarter) +
