...
m <- leaflet() %>%
  addTiles() %>%
  addMarkers(data = ws_raw, 
             lat = ~decimalLatitude, 
             lng = ~decimalLongitude,
             popup = paste0("Lat:", ws_raw$decimalLatitude,
                            "<br/>", 
                            "Long:", ws_raw$decimalLongitude))
m
```
...
whale_shark <- ws_raw %>%
                ...
                mutate(location = case_when(decimalLatitude == -22.6 & decimalLongitude == -113.65 ~ 'Open Range',
                                            ...
                                            TRUE ~ 'Coastal Area'),
                       season = case_when(month(eventDate) == c("12", "1", "2") ~ 'Summer',
                                          month(eventDate) == c("3", "4", "5") ~ 'Fall',
                                          month(eventDate) == c("6", "7", "8") ~ 'Winter',
                                          TRUE ~ 'Spring'))
...
# Converting whale shark dataset to sf object
ws_sf <- st_as_sf(whale_shark,
...
ws_transform <- st_transform(ws_sf, crs = st_crs(states))
...
ggplot() +
  ...
  ylim(-45, -5) +
  xlim(110, 155) +
  geom_point(data = whale_shark, aes(x = decimalLongitude, y = decimalLatitude)) +
  theme_bw()
```
