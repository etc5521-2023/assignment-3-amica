...
library(zoo) # update
library(tsibbletalk) # update
library(feasts) # update
library(plotly) # update
library(tibble) # update
library(forecast) # update
```
...
Tignostic analysis for tourism data to understand average trips taken by people to "Exmouth" for for various reasons.
...
ggplot(highest_sightings, aes(x = year_quarter, y = avg_trips)) +
  geom_line() +
  geom_point() +
  labs(title = "Average Trips Over Time",
       x = "Quarter",
       y = "Average trips") +
  theme_classic()
```
