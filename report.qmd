...
# NOT SURE WHAT THIS IS FOR, NEED TO RESEARCH
...
- According to the `BASIS_OF_RECORD_INVALID` column, there are `r ws_assert %>% filter(BASIS_OF_RECORD_INVALID == "TRUE") %>% nrow()` values recorded as TRUE indicating that they are invalid records. So, they are also removed.
...
- When plotting the locations of whale shark spotting, it is evident that some instances are from around the world and not just Australian waters. We can ignore the values which are far away from Australia since our expectations are only about analyzing these species in Australia. Some of the locations can be wrong as well and could be faulty data entry. For example, interchanging the sign of the longitude could take a point from Australia to South America.
