...
Before we can combine the joined weather and sighting dataset, we must first convert `the whale_shark_weather.rda` into a sf object so that there will be a common variable to join with the tourism dataset. 
...
Below is the code to combine both the whale_weather data with the tourism data. The merging of the datasets is done by joining them by polygon coordinates.
...
From all the tourist spots in Australia, we chose the tourist spot which contains the weather station **Learmonth Airport Weather Station**, and the sightings with the most occurrences of whale sharks which in this case is **Exmouth** which we were able to figure out after merging the two datasets. We also filtered out the Purpose of tourism with 'Total' since we it will just overlap the other purposes and provide inaccurate results while analyzing the results.

```{r}
#| label: fig-neartoursit
#| fig-cap: "Nearest Tourist Area"
#| echo: true
#| warning: false
#| message: false
#| eval: true
...
ggplotly(expect1 , tooltip = c("x", "y"))
...
ggplotly(expect3 , tooltip = c("x", "y"))
```
...
ggplotly(expect4 , tooltip = c("x", "y"))
```
...
Let's breakdown the distribution of each weather related variables from @fig-weatherdist. In terms of rain precipitation, it looks like the area around `Learmonth Airport` is mostly dry, concluded by the zero (0) precipitation throughout the year with raining happened occasionally. On the other hand, these area experienced a high variance of temperature, as can be seen, __the minimum temperature was 3.5 degree celcius and maximum temperature was 48.9 degree celcius__. The next analysis will cover the profile of the weather based on their trend over the time.
...
sightvstemp <- whale_shark_weather %>%
  filter(sighting == 1) %>%
...
ggplotly(sightvstemp , tooltip = c("x", "y"))
```
...
sightvsprcp <- whale_shark_weather %>%
  filter(sighting == 1) %>%
  group_by(eventDate, season) %>%
  summarise(sightings = sum(sighting),
            prcp = mean(prcp)) %>%
...
ggplotly(sightvsprcp , tooltip = c("x", "y"))
```
...
```{r}
#| label: fig-purpose
#| fig-cap: "Tourism Trends in Exmouth Based on Purpose"
#| echo: false
#| warning: false
#| message: false
...
p1 <- tourism_shared %>%
  ...
  geom_line(aes(group = Region), alpha = 0.5, color = "skyblue") +
...
The visualized analysis in @fig-purpose provides a comprehensive insight into the tourism trends in Exmouth, segmented by the purpose of visitation.

**Tourism Over Time by Purpose**

<div style="background-color: #FFFFCC; padding: 10px;"> 
- The first graph depicts Exmouth's tourism trends across various quarters from 1998, categorized by the purpose of the visit.

- A significant observation is the __continual increase in tourists visiting Exmouth for holidays__ compared to business travelers and other visitors, __highlighting its growing status as a favored vacation spot__.
</div>

**Seasonal versus Trend Strength**

- The second graph compares seasonal patterns and trends across different regions.

- Exmouth's analysis emphasizes its status as a popular holiday destination, with a consistent rise in vacationers, overshadowing business and family visitors.

<div style="background-color: #FFFFCC; padding: 10px;"> 
- Further analysis will concentrate on "Holiday" data to delve deeper into Exmouth's vacation trends.
</div>
...
tourismoverall <- ggplot(highest_sightings, aes(x = year_quarter, y = total_trips)) +
...
ggplotly(tourismoverall , tooltip = c("x", "y"))
```


- We first start off by visualizing the overall tourist visits for each year in the Exmouth region. From the plot above, we gain two valuable insights. The first being that __the average number of trips fluctuate frequently during a year which could be due to the quarter__. The second one being that __the peaks of the trips seems to be more in the second half of the plot__. 
...
```{r}
#| label: fig-tourismsmooth
#| fig-cap: "Spaghetti Plot For Tourist Trips in Exmouth"
#| echo: false
#| warning: false
#| message: false


tourismsmooth <- highest_sightings %>%
...
ggplotly(tourismsmooth , tooltip = c("x", "y"))
```

<div style="background-color: #FFFFCC; padding: 10px;"> 
As we can see from the above plot @fig-tourismsmooth, the thick blue line does indicate that the overall tourist trips have increased over the years, especially after 2010. However, it we look at the quarter lines themselves, we notice that the teal colored line is way higher than the rest. Could this be due to __the quarter being a either spring or winter where whale sharks are prominent as depicted  from our previous analysis on the sightings by seasons__.
</div>

```{r}
#| label: fig-tourismqua
#| fig-cap: "Total Trips in Exmouth By Quarter"
#| fig-width: 12
#| fig-height: 11
#| echo: false
#| warning: false
...
tourismquart <- highest_sightings %>%
  index_by(year_quarter) %>%
  summarise(total_trips = mean(total_trips)) %>%
    mutate(quarter = substr(year_quarter, nchar(year_quarter), nchar(year_quarter) + 2),
         year = substr(year_quarter, nchar(year_quarter)-4, nchar(year_quarter) )) %>%
...
ggplotly(tourismquart , tooltip = c("x", "y"))
```

<div style="background-color: #FFFFCC; padding: 10px;"> 
As we can see from the above @fig-tourismqua:

The __most visited season by tourists is Q3 (winter to spring) , followed by Q2 and Q4__.
</div>
...
> **Note:** **Due to severe missing data before 2010, our focus is on 2010-2023.**
...
<div style="background-color: #FFFFCC; padding: 10px;">
We have combined the number of tourists and the number of whale sharks to observe the relationship between them visually. The strong relationship between the number of travelers and the sighting of a whale shark cannot be perfectly explained due to a few possible reasons. 
...
But we still have found some interesting insights:

<div style="background-color: #FFFFCC; padding: 10px;">
- Travel trends in Exmouth highlight a preference for visiting between Q2 and Q4, with __Q3 being the peak season for sightseeing__, as indicated in the seasonal trend analysis.

- Despite the impact of the COVID-19 pandemic on global tourism, __the successful year of 2021 for tourism in Australia suggests a potential recovery__ in the sector.

- The __launch of the COVID-19 vaccination program by the Australian Government in December 2020 likely played a role in increasing travelers' confidence__, leading to a willingness to travel post-vaccination.

- The [2021-2022 Australian Tourism Promotion Plan](https://www.transparency.gov.au/sites/default/files/reports/2021-22_corporate_plan_tourism_australia.pdf), outlined on the Australian Tourism Board's website, encompasses a range of promotional measures initiated by the Australian government in 2021.

- These promotional efforts have significantly contributed to the growth and revival of the Australian tourism industry, providing a positive outlook for the sector.

</div>


__In next section, we combine the weather, previous traveler information and Whale Shark data to determine if any correlations exist among them.__
...
```{r}
#| label: fig-ggpairs
#| fig-cap: "Correlations Between Sightings,Temperature,Precipitation,Tourist Trips"
#| echo: false
#| warning: false
#| message: false
...
<div style="background-color: #a3c1ad; padding: 10px;">
Based on our data, we can provide the following advice to ecotourists:

- **Best Time to Travel:** It is advisable for ecotourists to plan their trips during Q2 to Q4 when the weather is moderate and rainfall is lower, which is conducive to the development and observation of the ecosystem.
...
Through these suggestions, we hope ecotourists can better understand how to respect and protect the ecological environment of the places they visit, while enjoying a meaningful and responsible travel experience.
</div>
...
-  **Kendall**:   McLeod A (2022). _Kendall: Kendall Rank Correlation and Mann-Kendall Trend Test_. R package version
